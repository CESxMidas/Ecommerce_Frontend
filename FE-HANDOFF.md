...
Nếu lỗi → cân nhắc simplify giống `side-drawer.tsx`.
...
|---------|------|---------|
| Trung bình | Google OAuth | Cần `GOOGLE_CLIENT_ID/SECRET`, callback `/api/next-auth/callback/google` |
...
| Thấp | Blog field `category` từ BE | Badge category trên card |
| Thấp | Static pages từ CMS API | Thay `static-pages.ts` |
| Thấp | Copy tiếng Việt | Parity legacy nếu cần |
| Thấp | Rà soát `Product` vs `NormalizedProduct` | Types |
...
| Dev CORS | `Ecommerce_Backend/.env` → `CORS_ORIGIN=http://localhost:3000` |
